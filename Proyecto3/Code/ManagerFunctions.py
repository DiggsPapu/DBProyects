...
def reportery(cursor):
    while (True):
        option = input("Select between : \n1- Get the top 10 sesions with more users\n2- Amount of sesions and users for category in a range of dates given\n3- Top 5 prefered instructors\n4- The amount of accounts created in the last 6 months\n5- For an specific date the top hour\n6- Exit\n")
        if (option == "1"):
            topTen(cursor)
        elif (option == "2"):
            initdate = enterDate()
            lastdate = enterDate()
            print("Sesion category:\n")
            amountSesionCat(initdate,lastdate,cursor)
            print("User category:\n")
            amountUserCat(initdate,lastdate,cursor)
        elif (option == "3"):
            topFiveIns(cursor)
        elif (option == "4"):
            amoountOfDiamond(cursor)
        elif (option == "5"):
            picoHour(cursor)
        elif (option == "6"):
            break
...
def picoHour(cursor):
    date = enterDate()
    cursor.execute('''select count(uc.userid), s.sesionhour from sesion as s inner join (select userid, sesioncode from sesionuser) as uc on uc.sesioncode = s.sesioncode where s.sesiondate = '{}' group by s.sesionhour order by count(uc.userid) desc Limit 1'''.format(date))
    print("The pick hour is "+ str(cursor.fetchone()[1]))
...
def searchInstructor(username, password, cursor):
    try:
        ...
        return None
def topTen(cursor):
    cursor.execute('''select * from sesion inner join (select count(sesionuser.userid), sesionuser.sesioncode from sesionuser group by sesionuser.sesioncode order by count(sesionuser.userid) desc limit 10 ) as us on us.sesioncode = sesion.sesioncode''')
    sesions = cursor.fetchall()
    for x in sesions:
        print("Sesioncode: "+str(x[0])+" About: "+str(x[1])+" Date: "+str(x[2])+" Hour: "+str(x[3]))
def amountSesionCat(initdate, lastdate,cursor):
    cursor.execute('''select count(sesion.sesioncode), c.category from sesion inner join (select category, categorycode from excategory) as c on c.categorycode = sesion.categorycode where sesion.sesiondate<='{}' and sesion.sesiondate>= '{}' group by c.category, sesion.categorycode'''.format(lastdate, initdate))
    sesions = cursor.fetchall()
    for x in sesions:
        print("Number of sesions: "+str(x[0])+" Category: "+str(x[1]))
def amountUserCat(initdate, lastdate,cursor):
    cursor.execute('''select count (uc.userid), c.category from sesionuser as uc inner join (select sesioncode, categorycode from sesion where sesiondate>='{}' and sesiondate<='{}') as s on s.sesioncode = uc.sesioncode inner join (select category, categorycode from excategory) as c on c.categorycode = s.categorycode group by c.category, s.categorycode'''.format(initdate, lastdate))
    sesions = cursor.fetchall()
    for x in sesions:
        print("Number of users: "+str(x[0])+" Category: "+str(x[1]))
def topFiveIns(cursor):
    cursor.execute('''select workername, count(s.workerid) from worker inner join (select s.sesioncode, s.workerid, uc.userid from sesion as s inner join (select userid, sesioncode from sesionuser ) as uc on uc.sesioncode = s.sesioncode) as s on s.workerid=worker.workerid group by workername Limit 5''')
    sesions = cursor.fetchall()
    for x in sesions:
        print("Number of users: "+str(x[1])+" Name of instructor: "+str(x[0]))
def amoountOfDiamond(cursor):
    cursor.execute('''select subscriptiontype, count(subscriptiontype) from usercontract where initdate>='2022-06-18' and subscriptiontype = 'Diamond' group by subscriptiontype ''')
    sesions = cursor.fetchall()
    for x in sesions:
        print("Number of accounts: "+str(x[1])+ " Type of account: "+ str(x[0]))
def enterDate():
    while (True):
