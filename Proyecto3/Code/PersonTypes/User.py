...
class User:
    ...
    def assignSesionRandom(self):
            self.cursor.execute('''select sesion.sesioncode from sesion
            except all
            (select sesion.sesioncode from sesion
            inner join sesionuser 
            on sesionuser.sesioncode = sesion.sesioncode
            where sesionuser.userid = '{}')'''.format(self.userid)) 
            values = self.cursor.fetchall()
            elec = str(random.choice(values)[0])
            self.cursor.execute('''insert into sesionuser (userid,sesioncode)
                values('{}',{})'''.format(self.userid,elec))
            print('''
        The user {} assigned to the sesion {} '''.format(self.userid,elec))
    ...
    def updateSesionValuesRandom(self):
        self.cursor.execute('''select * from sesion where sesion.sesionstatus = 'DONE' ''')
        sesions = self.cursor.fetchall()
        for sesion in sesions:
            self.cursor.execute('''update sesionuser set caloriesacomplished = {}, heartrate = {} where userid = '{}' and sesioncode = {} '''.format( str(random.randint(0,1000)), str(random.randint(30,200)), self.userid, str(sesion[0]) ))
