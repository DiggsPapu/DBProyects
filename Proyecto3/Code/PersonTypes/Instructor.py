...
class Instructor(Worker):  # type: ignore
    ...
    def createRandomSesion(self,name,date_val,hour,timelength,category_code):
        self.cursor.execute('''insert into sesion(sesionname,sesiondate,sesionhour,sesionstatus,timelength,description,workerid,categorycode)
        values('{}','{}','{}','WAIT','{}','{}','{}',{})'''.format(name,str(date_val),str(hour),timelength.__str__(),"any session",self.worker_id,str(category_code)))
    ...
    def updateSesionStatus(self,date):
        self.cursor.execute('''update sesion set sesionstatus = 'DONE' where sesion.sesiondate<='{}' '''.format(date))