...
def checkUpdateInfo(user, cursor):
    ...
    cursor.execute('''select * from sesionuser inner join (select sesioncode, sesiondate, sesionhour from sesion where sesion.sesionstatus = 'done') as s on s.sesioncode = sesionuser.sesioncode where sesionuser.userid = \'{}\''''.format(user[0]))
    sesions = cursor.fetchall()
    for x in sesions:
